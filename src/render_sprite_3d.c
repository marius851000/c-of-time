...
#include "smalllib/fragment_size_lookup.h"
...
void render_fragment(struct animation_control* anim_ctrl, struct wan_fragment* fragment, uint32_t fragment_pos) {
    ...
    struct vec2_16 size_tex_indice;
    get_size_tex_indices_from_sprite_indices(fragment->shape_indice, fragment->size_indice, &size_tex_indice);
    ...
    //TODO: only work for 16-bit sprites for now (such as all Pokémon sprites)
    //TODO: the second-to-last bit control the transparency. Set to 0 for now for debuggin purpose.
    //TODO: flipping on both axis
    GeomSetTexImageParam(3, 0, size_tex_indice.x, size_tex_indice.y, 0, 0, 0, 0x11000);

    DebugPrint(2, "The fragment byte is at %i", anim_ctrl->wan_fragments_byte_store[fragment->fragment_bytes_index]);
    // send mesh
